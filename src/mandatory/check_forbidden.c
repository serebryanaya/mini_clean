...
/*   Updated: 2022/02/19 18:35:10 by pveeta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_status check_forbidden2(char *str, t_input *input, U_INT *i)
{
	...
	if (str[*i] == '!' || str[*i] == '>' || str[*i] == '<')
	{
		...
		// '<<<' '<<< '
		else if (str[*i + 1] && str[*i + 2] && str[*i + 3] && (!(str[*i + 4]) || (str[*i + 4] \
		&& str[*i + 4] == ' ')) && str[*i + 1] == str[*i + 2] && str[*i + 2] == str[*i + 3] && str[*i + 1] == '<')
			input->token = modif_strdup("newline", input);
		// '>>>' '>>> '
		else if (str[*i + 1] && str[*i + 2] && str[*i + 3] && (!(str[*i + 4]) || (str[*i + 4] \
		&& str[*i + 4] == ' ')) && str[*i + 1] == str[*i + 2] && str[*i + 2] == str[*i + 3] && str[*i + 1] == '>')
		...
		// '>>>>' '>>>>>' '>>>> '
		else if (str[*i + 1] && str[*i + 2] && str[*i + 3] && str[*i + 4] && \
		str[*i + 1] == str[*i + 2] && str[*i + 2] == str[*i + 3] && str[*i + 3] == str[*i + 4] \
		&& str[*i + 1] == '>')
			input->token = modif_strdup(">>", input);
		// '<<<<<' '<<<<< '
		else if (str[*i + 1] && str[*i + 2] && str[*i + 3] && str[*i + 4] && str[*i + 5] && \
		(!(str[*i + 6]) || (str[*i + 6] && str[*i + 6] == ' ')) && str[*i + 1] == str[*i + 2] \
		&& str[*i + 2] == str[*i + 3] && str[*i + 3] == str[*i + 4] && str[*i + 4] == str[*i + 5] \
		&& str[*i + 1] == '<')
			input->token = modif_strdup("<<", input);
		// '<<<<<<' '<<<<<< '
		else if (str[*i + 1] && str[*i + 2] && str[*i + 3] && str[*i + 4] && str[*i + 5] && \
		str[*i + 6] && str[*i + 1] == str[*i + 2] && str[*i + 2] == str[*i + 3] && \
		str[*i + 3] == str[*i + 4] && str[*i + 4] == str[*i + 5] && str[*i + 5] == str[*i + 6] \
		&& str[*i + 1] == '<')
			input->token = modif_strdup("<<<", input);
		...
		// else if (str[*i + 1] && str[*i + 2] && str[*i + 3] && str[*i + 4] && \ // '<<<<<' '<<<<<<<'
		// (!(str[*i + 4]) || str[*i + 4] && str[*i + 4] == ' ') && str[*i + 1] == str[*i + 2] \
		// && str[*i + 2] == str[*i + 3] && str[*i + 3] == str[*i + 4] \
		// && str[*i + 1] == '<<')
		// 	input->token = modif_strdup("<", input);


		// else if (str[*i + 1] && str[*i + 2] && str[*i + 3] && !(str[*i + 4]) && \
		// str[*i + 1] == str[*i + 2] && str[*i + 2] == str[*i + 3] && str[*i + 1] == '>')
		// 	input->token = modif_strdup(">>", input);
